...
var deepMergeConfig = (globalConfig, localConfig) => {
  ...
  const result = (0, import_utils.deepMerge)((0, import_utils.filterUndefinedVal)(newGlobalConfig), (0, import_utils.filterUndefinedVal)(newLocalConfig));
...
var generateAppOptions = (appName, garfish, options) => {
  ...
  const insulationVariable = Array.from(/* @__PURE__ */ new Set([
    ...
    ...(appInfo == null ? void 0 : appInfo.insulationVariable) || []
  ]));
...
function appLifecycle() {
  return new import_hooks.PluginSystem({
    ...
    customRender: new import_hooks.SyncWaterfallHook("customRender")
  });
}

// src/module/esModule.ts
var import_es_module_lexer = require("@alioth-org/es-module-lexer");
...
${sourcemap}`);
  ...
  async analysisModule(lockId, code, envVarStr, noEntryEnvVarStr, baseUrl, realUrl) {
    ...
    await (0, import_es_module_lexer.init)();
...
var App = class {
  constructor(context, appInfo, entryManager, resources, isHtmlMode, customLoader) {
    ...
    for (const key in nodes) {
      nodes[key].forEach((node) => {
        ...
        if ((0, import_utils4.isGarfishConfigType)({
  ...
  addSourceList(sourceInfo) {
    ...
    if (Array.isArray(sourceInfo)) {
      const nSourceList = sourceInfo.filter((item) => {
        const url = (0, import_utils4.getSourceURL)(item.url);
      ...
      const url = (0, import_utils4.getSourceURL)(sourceInfo.url);
  ...
  runCode(code, env, url, options) {
    if (options && options.isModule) {
      ...
      const revertCurrentScript = (0, import_utils4.setDocCurrentScript)(this.global.document, code, true, url, options == null ? void 0 : options.async, options == null ? void 0 : options.defer, options == null ? void 0 : options.originScript);
      code += url ? `
//# sourceURL=${url}
` : "";
        ...
        if (jsManager[type]) {
          try {
            ...
            if (type === "defer") {
              ...
              if (node) {
                noEntry = (0, import_utils4.toBoolean)(this.entryManager.findAttributeValue(node, "no-entry"));
              }
              if (!noEntry) {
                noEntry = (0, import_utils4.toBoolean)(this.isNoEntryScript(targetUrl));
              }
            }
    ...
    const customRenderer = {
      ...
      script: (node) => {
        ...
        if (this.appInfo.sandbox) {
          ...
          if (scriptUrl && ((_a = this.appInfo.sandbox) == null ? void 0 : _a.excludeAssetFilter) && ((_b = this.appInfo.sandbox) == null ? void 0 : _b.excludeAssetFilter(scriptUrl))) {
            return DOMApis.createElement(node);
          }
        }
        ...
        if (jsManager) {
          if (jsManager.defer) {
            ...
            this.execScript(scriptCode, {}, targetUrl, {
              ...
              defer: false,
              isInline: jsManager.isInlineScript(),
              noEntry: (0, import_utils4.toBoolean)(entryManager.findAttributeValue(node, "no-entry")) || (0, import_utils4.toBoolean)(this.isNoEntryScript(targetUrl)),
 ...
 ${tipInfo}`);
      ...
      link: (node) => {
        if (this.appInfo.sandbox && typeof this.appInfo.sandbox === "object" && this.appInfo.sandbox.disableLinkTransformToStyle) {
          return DOMApis.createElement(node);
        }
...
function fetchStaticResources(appInfo, loader, entryManager, sandboxConfig) {
  const toBoolean2 = (val) => typeof val !== "undefined" && val !== "false";
  const jsNodes = Promise.all(entryManager.findAllJsNodes().filter((node) => {
    ...
    if (src) {
      ...
      return loader.load({
        ...
        if (jsManager) {
          ...
          (0, import_utils5.warn)(`[${appInfo.name}] Failed to load script: ${fetchUrl}`);
        }
      }).catch(() => null);
  ...
  const linkNodes = Promise.all(entryManager.findAllLinkNodes().map((node) => {
    if (!entryManager.DOMApis.isCssLinkNode(node))
      return;
    if (appInfo.sandbox && typeof appInfo.sandbox === "object" && appInfo.sandbox.disableLinkTransformToStyle)
      return;
    ...
    if (href) {
      ...
      return loader.load({ scope: appInfo.name, url: fetchUrl }).then(({ resourceManager: styleManager }) => {
        if (styleManager) {
          styleManager.setDep(node);
          styleManager == null ? void 0 : styleManager.correctPath();
          return styleManager;
        } else {
          (0, import_utils5.warn)(`${appInfo.name} Failed to load link: ${fetchUrl}`);
        }
      }).catch(() => null);
    }
  }).filter(Boolean));
...
async function processAppResources(loader, appInfo) {
  ...
  if (entryManager instanceof loader.TemplateManager) {
    isHtmlMode = true;
    const [js, link, modules] = await fetchStaticResources(appInfo, loader, entryManager, appInfo.sandbox);
    resources.js = js;
    resources.link = link;
  ...
  safeLoad({
    ...
    callback: (manager) => {
      const loadStaticResource = () => {
        if (manager instanceof import_loader.TemplateManager) {
...
var Garfish = class extends import_eventemitter2.EventEmitter2 {
  constructor(options) {
    ...
    this.loader = new import_loader2.Loader();
