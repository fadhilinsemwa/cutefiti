...
export type Action<State, Payload = any> = (state: State, payload: Payload, ...extraArgs: any[]) => State | void;
...
type Merge<T extends Record<string, any>, K extends string> = { [key in keyof T as K]: T[key] };
...
export interface Computed<State> {
  [key: string]: ((state: State) => any) | [...s: Model[], selector: (state: State, ...args: any) => any];
}
type GetComputedFromArray<A> = A extends {
  _: object;
} ? never : A extends (...args: any) => infer R ? R : any;
/**
 * GetComputed
 * A type hook to resolve computed type from  Model
 */
export interface GetComputed<M extends Model, C = M['_']['computed']> {
  computed: { [K in keyof C]: C[K] extends (...args: any[]) => infer R ? R : C[K] extends Array<infer A> ? GetComputedFromArray<A> : K };
}
export type GetModelState<M extends Model = Model> = GetComputed<M>['computed'] extends Record<string, any> ? UnionToIntersection<Merge<GetState<M>, 'state'>['state'] | Merge<GetComputed<M>, 'computed'>['computed']> : Merge<GetState<M>, 'state'>['state'];
export type GetModelActions<M extends Model = Model> = UnionToIntersection<Merge<GetActions<M>, 'actions'>['actions']>;
 ...
 * }
 */
type ExtractDispatchActions<A extends Actions<any> = Actions<any>> = { [key in keyof A]: A[key] extends Actions<any> ? ExtractDispatchActions<A[key]> : A[key] extends () => any ? () => ReduxAction<undefined> : A[key] extends (S: any, ...args: infer S) => any ? (...args: S) => ReduxAction<S[0]> : never };
type ModelType = Model;
type ExcludeEmpty<T> = T extends infer P & {} ? P : T;
type NeededUnionType<T extends any[]> = T[number];
type GetModelsState<Models extends Model[]> = ExcludeEmpty<UnionToIntersection<NeededUnionType<{ [Index in keyof Models]: MountedModel<Models[Index]>['state'] }>>>;
type GetModelsAction<Models extends Model[]> = Models extends [infer Head, ...infer Tail] ? (Head extends Model ? UnionToIntersection<Merge<GetActions<Head>, 'actions'>['actions']> : Record<string, unknown>) & (Tail extends Model[] ? GetModelsAction<Tail> : Record<string, unknown>) : Record<string, unknown>;
type GetModelsStateTuple<Models extends ModelType[], U extends any[] = []> = Models extends [infer Head, ...infer Tail] ? Head extends ModelType ? Tail extends ModelType[] ? GetModelsStateTuple<Tail, [...U, GetModelsState<[Head]>]> : [...U, GetModelsState<[Head]>] : U : U;
type GetModelsActionsTuple<Models extends ModelType[], U extends any[] = []> = Models extends [infer Head, ...infer Tail] ? Head extends ModelType ? Tail extends ModelType[] ? GetModelsActionsTuple<Tail, [...U, GetModelsAction<[Head]>]> : [...U, GetModelsAction<[Head]>] : U : U;
