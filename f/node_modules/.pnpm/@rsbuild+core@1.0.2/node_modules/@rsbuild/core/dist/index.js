...
var init_restart = __esm({
  "src/server/restart.ts"() {
    ...
    restartDevServer = async ({
      ...
      logger.info(`Restart because ${color4.yellow(filename)} is changed.
`);
      ...
      setup(api) {
        ...
        api.onBeforeCreateCompiler(({ bundlerConfigs }) => {
          if (bundlerConfigs.every((config) => !config.entry)) {
            throw new Error(
              `Could not find any entry module, please make sure that ${color11.cyan(
                "src/index.(ts|js|tsx|jsx|mjs|cjs)"
              )} exists, or customize entry through the ${color11.cyan(
                "source.entry"
              )} configuration.`
            );
          }
...
function getHeader(longestFileLength, longestLabelLength, options) {
  ...
  return color12.bold(color12.blue(headerRow));
}
async function printFileSizes(options, stats, rootPath) {
  ...
  if (options.total !== false) {
    const totalSizeLabel = `${color12.bold(
      color12.blue("Total size:")
    )}  ${calcFileSize(totalSize)}`;
    let log = `
  ${totalSizeLabel}
`;
    if (options.compressed) {
      const gzippedSizeLabel = `${color12.bold(
        color12.blue("Gzipped size:")
  ...
  "src/plugins/fileSize.ts"() {
    ...
    coloringAssetName = (assetName) => {
      if (JS_REGEX.test(assetName)) {
        return color12.cyan(assetName);
      }
      if (CSS_REGEX.test(assetName)) {
        return color12.yellow(assetName);
      }
      if (HTML_REGEX.test(assetName)) {
        return color12.green(assetName);
      }
      return color12.magenta(assetName);
    };
    pluginFileSize = () => ({
        ...
        api.onAfterBuild(async ({ stats, environments, isFirstCompile }) => {
          ...
          await Promise.all(
            Object.values(environments).map(async (environment, index) => {
              ...
              const name = color12.green(environment.name);
        ...
        const getCleanPath = (environment) => {
          ...
          if (cleanDistPath === "auto") {
            ...
            if (!cleanDistPath) {
              ...
              logger.warn(
                `Please set ${color13.yellow("`output.cleanDistPath`")} config manually.`
              );
              logger.warn(`Current root path: ${color13.dim(rootPath)}`);
              logger.warn(`Current dist path: ${color13.dim(distPath)}`);
            }
          }
          if (cleanDistPath) {
            return distPath;
...
async function getTemplate(entryName, config, rootPath) {
  ...
  if (!existTemplatePath.has(absolutePath)) {
    if (!await isFileExists(absolutePath)) {
      throw new Error(
        `Failed to resolve HTML template, please check if the file exists: ${color14.cyan(
          absolutePath
        )}`
      );
    }
    ...
    try {
      ...
      if (rsbuild) {
        ...
        if (!existsSync(distPath)) {
          throw new Error(
            `The output directory ${color16.yellow(
              distPath
