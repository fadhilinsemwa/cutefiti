...
var import_browserslist_to_es_version = require("browserslist-to-es-version");
...
function displayCodePointer(code, pos) {
  ...
  const arrowLine = import_picocolors.default.yellow("^".padStart(arrowPos + 11, " "));
  return `${subCode}
${arrowLine}`;
}
...
function makeCodeFrame(lines, highlightIndex) {
  ...
  for (let i = startLine; i < endLine; i++) {
    if (i === highlightIndex) {
      ...
      ret.push(import_picocolors.default.yellow(`${lineNumber} | ${lines[i]}`));
    } else {
      const lineNumber = ` ${i + 1}`.padStart(6, " ");
      ret.push(import_picocolors.default.gray(`${lineNumber} | ${lines[i]}`));
    }
  }
  return `
${ret.join("\n")}`;
...
async function tryGenerateErrorFromSourceMap({
  ...
  try {
    const sourcemap = await import_node_fs.default.promises.readFile(sourceMapPath, "utf-8");
...
function printErrors(errors, ecmaVersion) {
  if (errors.length === 0) {
    import_core.logger.success("[@rsbuild/plugin-check-syntax] Syntax check passed.");
    return;
  }
  ...
  const expectedVersion = import_picocolors2.default.yellow(`ecmaVersion <= ${ecmaVersion}`);
...
`
  ...
  errs.forEach((err, index) => {
    console.log(import_picocolors2.default.bold(import_picocolors2.default.red(`  ERROR ${index + 1}`)));
...
function printMain(error, longest) {
  ...
  for (const [key, content] of Object.entries(error)) {
    ...
    const title = import_picocolors2.default.magenta(`${fillWhiteSpace(`${key}:`, longest + 1)}`);
...
var CheckSyntaxPlugin = class _CheckSyntaxPlugin {
  constructor(options) {
    ...
    this.ecmaVersion = options.ecmaVersion || (0, import_browserslist_to_es_version.browserslistToESVersion)(this.targets);
  }
  apply(compiler) {
    compiler.hooks.afterEmit.tapPromise(
      ...
      async (compilation) => {
        ...
        const emittedAssets = compilation.getAssets().filter((a) => a.source).map((a) => {
          ...
          return (0, import_node_path.resolve)(outputPath, resourcePath);
        });
  ...
  async check(filepath) {
    ...
    if (JS_REGEX.test(filepath)) {
      const jsScript = await import_node_fs3.default.promises.readFile(filepath, "utf-8");
      await this.tryParse(filepath, jsScript);
    }
  }
  async tryParse(filepath, code) {
    try {
      (0, import_acorn.parse)(code, { ecmaVersion: this.ecmaVersion });
    } catch (_) {
      const err = _;
